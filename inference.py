...
def main():
    ...
    # Load input images
    input_images = [Image.open(p).convert("RGB").resize((512, 512)) for p in config.images_path]
    ...
    print("Generated image saved to:", out_path)
    ...
    clip_t = compute_clip_text_similarity(
        clip_model, clip_processor, gen_image, config.prompt, args.device
    )

    relation_word = extract_relation_word(config.prompt)
