...
class MaskedIPAttnProcessor2_0(nn.Module):
    ...
    def __init__(self, hidden_size, cross_attention_dim=None, scale=0.6, num_tokens=4, text_tokens=77,
                 need_text_attention_map=False, need_image_attention_map=True, num_dummy_tokens=4, mask_threshold=0.5,
                 use_psuedo_attention_mask=False, subject_scales=None, attn_store=None, place_in_unet=None, start_step=5):
        super().__init__()
    ...
    def get_predicate_attention_maps(self, attention_probs, boxes, phrase_idxes, head_size):
        phrase_idxes = phrase_idxes.squeeze(1)
        bsz = boxes.shape[0]
        _, num_tokens_q, num_tokens_k = attention_probs.shape
        attention_probs = attention_probs.view(bsz, head_size, num_tokens_q, num_tokens_k)
        h = w = int(num_tokens_q ** 0.5)
        ...
        for i in range(bsz):
            sample_attention_maps = []
            start_idx, end_idx = int(phrase_idxes[i, 0].item()), int(phrase_idxes[i, 1].item())
            if start_idx == 0 and end_idx == 0:
                sample_attention_maps.append(
                    torch.zeros(num_tokens_q, dtype=attention_probs.dtype, device=attention_probs.device))
            else:
        ...
        if num_tokens_q <= 32 ** 2 and self.attn_store is not None:
            self.attn_store[self.place_in_unet] = torch.stack(batch_attention_maps).reshape(bsz, h, w)
        else:
            self.attention_maps.append(torch.stack(batch_attention_maps).reshape(bsz, h, w))
        # self.attention_maps.append(torch.stack(batch_attention_maps).reshape(bsz, h, w))
    ...
    def __call__(
        ...
        if not self.need_text_attention_map:
            ...
            self.get_predicate_attention_maps(attention_probs, boxes, phrase_idxes, attn.heads)
        ...
        ip_key = self.to_k_ip(ip_hidden_states.to(self.to_k_ip.weight.data.dtype)).to(query.dtype)
        ip_value = self.to_v_ip(ip_hidden_states.to(self.to_v_ip.weight.data.dtype)).to(query.dtype)
